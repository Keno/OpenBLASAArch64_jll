    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("OpenBLAS_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "OpenBLAS_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"OpenBLAS", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "OpenBLAS_jll", "src", "OpenBLAS_jll.jl"))
        @info("OpenBLAS_ll dev'ed out to /home/keno/.julia/dev/OpenBLAS_jll with pre-populated override directory")
    end
end
